...
var layerin = null;
$(function() {
    toogle("overlappingDonorAnalysis");
    initCitys();
    //setTime();
...
function initCitys() {
    ...
    $.get("overlappingDonorAnalysis/getCitys", null, function(data) {
...
function setTime() {
    ...
    $.post("overlappingDonorAnalysis/getDataGroupByDate", params, function(data) {
        data = JSON.parse(data);
        var sdata = [];
...
function queryTrail() {
    ...
    queryBtn.start();
    $.post("overlappingDonorAnalysis/getData", params, function(data) {
        data = JSON.parse(data);
...
function initMap(mapId) {
    ...
    $.ajax({
        type: "GET",
        url: "overlappingDonorAnalysis/switchSite",
        dataType: "text",
        beforeSend: function() {
...
function drawMap(data, ecgiNc) {
    var targetCell = data.targetCell;
    var otherCells = data.otherCells;
    var vdata = [];
    var count;
    for (var i in otherCells) {
        // count = otherCells[i].ecgi == ecgiNc ? 100 : 30;
        if ((otherCells[i].ecgi && otherCells[i].ecgi.indexOf(ecgiNc) >= 0) || (ecgiNc && ecgiNc.indexOf(otherCells[i].ecgi) >= 0)) {
            count = 100;
        } else {
            count = 30;
        }
                ...
                click: function(e, data1) {
                    ...
                    for (var i = 0; i < cells.length; i++) {
                        for (var j = 0; j < data.tableData.length; j++) {
                            if (cells[i].indexOf(data.tableData[j].ecgiNc) >= 0 || (data.tableData[j].ecgiNc && data.tableData[j].ecgiNc.indexOf(cells[i]) >= 0)) {
