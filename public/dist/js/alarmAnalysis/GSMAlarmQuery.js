$(document).ready(function () {
	toogle("GSMAlarmQuery");
	initCitys();
	setTime();
	$("#exportBtn").on("click", function () {
		fileSave("file");
	});
	$("select#GSMAlarm").change(function(){
		var type =$("#GSMAlarm").val();
		if(type =="GSMCurrent"){
			$("#GSMTime").hide();
			$("#GSMAlarmManage").val("GSMCurrent");
		}else if(type =="GSMHistory"){
			$("#GSMAlarmManage").val("GSMHistory");
			$("#GSMTime").show();
		}
	});
...
function initCitys() {
	...
	var url = "GSMAlarmQuery/getCitys";
...
function query() {
	...
	var GSMType  = $("#GSMAlarmManage").val();

	if(GSMType == "GSMCurrent"){
		var params = {
			placeDim: placeDim,
	...
	exportBtn.start();
	$("#GSMAlarmTable").grid("destroy", true, true);
	var grid = $("#GSMAlarmTable").grid({
		columns: fieldArr,
		dataSource: {
			url: "GSMAlarmQuery/getTableData",
	...
	var url = "GSMAlarmQuery/getAllTableData";
...
function setTime() {
	...
	$.get("GSMAlarmQuery/getGSMAlarmTime", params, function (data) {
		var sdata = [];
