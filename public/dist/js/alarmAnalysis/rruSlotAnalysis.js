...
function getAllCity(){
	...
	var url = "rruSlotAnalysis/getAllCity";
	$.ajax({
		type:"post",
		url:url,
		dataType:"json",
...
function searchSlot(){
	$("#currSlotNum").html("");
	$("#currTime").html("");
	doSearchAllSlot("rruSlotAnalysis/getAllSlot","#slotTable");//板卡串号列表
	doSearchAllSlot("rruSlotAnalysis/getDisappearSlot","#disappearSlotTable");//消失板卡串号
}
...
function exportAllSlot(){
	...
	$.post("rruSlotAnalysis/exportAllSlot", params, function (data) {
		E.stop();
		if (data.result) {
...
function exportDisappearSlot(){
	...
	$.post("rruSlotAnalysis/exportDisappearSlot", params, function (data) {
		E.stop();
		if (data.result) {
			fileDownload(data.fileName);
		} else {
...
function slotTrendChart(serialNumber,block){
	$.ajax({
		type: "post",
		url: "rruSlotAnalysis/getSlotTrendChart",
...
function getOneSlotInfo(serialNumber){
	...
	$.post("rruSlotAnalysis/getOneSlotInfo", params, function(data){
		S.stop();
		if (data["data"]["total"] == 0) {
			layer.open({
				title: "提示",
		...
		var grid = $("#resultTable").grid({
			columns:fieldArr,
			dataSource:{
				url:"rruSlotAnalysis/getOneSlotInfo",
				success:function(data){
					grid.render(data["data"]);
				},
				type:"post"
...
function exportOneSolt(){
	...
	$.post("rruSlotAnalysis/exportOneSolt",{"serialNumber":serialNumber},function(data){
		E.stop();
		if (data.result) {
			fileDownload(data.fileName);
		} else {
