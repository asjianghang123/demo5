$(document).ready(function () {
	...
	$("select#LteAlarm").change(function(){
		var type =$("#LteAlarm").val();
		if(type =="LteCurrent"){
			$("#LteTime").hide();
			$("#LteAlarmManage").val("LteCurrent");
		}else if(type =="LteHistory"){
			$("#LteAlarmManage").val("LteHistory");
			$("#LteTime").show();
		}
	});
...
function initCitys() {
	...
	var url = "LteAlarmQuery/getCitys";
...
function query() {
	...
	var LteType  = $("#LteAlarmManage").val();
	if(LteType=="LteCurrent"){
	...
	exportBtn.start();
	$("#LteAlarmTable").grid("destroy", true, true);
	var grid = $("#LteAlarmTable").grid({
		columns: fieldArr,
		dataSource: {
			url: "LteAlarmQuery/getTableData",
	...
	var url = "LteAlarmQuery/getAllTableData";
...
function setTime() {
	...
	$.get("LteAlarmQuery/getLteAlarmTime", params, function (data) {
		var sdata = [];
