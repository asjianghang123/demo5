...
function getAllCity(){
	...
	var url = "BoardAnalysis/getAllCity";
	$.ajax({
		type:"post",
		url:url,
		dataType:"json",
...
function searchSlot(){
	$("#currSlotNum").html("");
	$("#currTime").html("");
	doSearchAllSlot("BoardAnalysis/getAllSlot","#slotTable");//板卡串号列表
	doSearchAllSlot("BoardAnalysis/getDisappearSlot","#disappearSlotTable");//消失板卡串号
}
function doSearchAllSlot(url, tableId){
	...
	$.post(url, params, function(data){
		...
		if (data["data"]["total"] == 0) {
			...
			}else if (tableId == "#disappearSlotTable") {
				layer.open({
				  	title: "提示",
				  	content: "消失板卡串号没有记录"
				});
...
function exportAllSlot(){
	...
	$.post("BoardAnalysis/exportAllSlot", params, function (data) {
		E.stop();
		if (data.result) {
...
function exportDisappearSlot(){
	...
	$.post("BoardAnalysis/exportDisappearSlot", params, function (data) {
		E.stop();
		if (data.result) {
			fileDownload(data.fileName);
		} else {
...
function slotTrendChart(serialNumber,block){
	$.ajax({
		type: "post",
		url: "BoardAnalysis/getSlotTrendChart",
...
function getOneSlotInfo(serialNumber){
	...
	$.post("BoardAnalysis/getOneSlotInfo", params, function(data){
		S.stop();
		if (data["data"]["total"] == 0) {
			layer.open({
			  title: "提示",
		...
		var grid = $("#resultTable").grid({
			columns:fieldArr,
			dataSource:{
				url:"BoardAnalysis/getOneSlotInfo",
				success:function(data){
					grid.render(data["data"]);
				},
				type:"post"
...
function exportOneSolt(){
	...
	$.post("BoardAnalysis/exportOneSolt",{"serialNumber":serialNumber,"type":type},function(data){
		E.stop();
		if (data.result) {
			fileDownload(data.fileName);
		} else {
