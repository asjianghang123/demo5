$(function () {
	...
	$.get("A2ThresholdAnalysis/getDate", params, function (data) {
		var sdata = [];
...
function getAllCity() {
	...
	var url = "A2ThresholdAnalysis/getAllCity";
	$.ajax({
		type: "GET",
		url: url,
		dataType: "json",
...
function query() {
	...
	$.get("A2ThresholdAnalysis/getMreA2ThresholdDataHeader", params, function (data) {
		if (data.error == "error") {
			layer.open({
				title: "提示",
				content: "数据不存在，请重新选择！"
		...
		$("#A2ThresholdTable").grid("destroy", true, true);
		var grid = $("#A2ThresholdTable").grid({
			columns: fieldArr,
			params: params,
			dataSource: {
				url: "A2ThresholdAnalysis/getMreA2ThresholdData",
				success: function (data) {
					data = eval("(" + data + ")");
					if (data.error == "error") {
						$("#A2ThresholdTable").grid("destroy", true, true);
...
function exportFile() {
	...
	queryBtn.start();
	exportBtn.start();
	var url = "A2ThresholdAnalysis/getAllMreA2ThresholdData";
