...
function newSiteSearch(){
    ...
    var fieldArr = [];
        $.post("newSite/getTableField", params, function (data) {
            if (data.result == "error") {
                l.stop();
                $("#newSiteTable").bootstrapTable("destroy");
                layer.open({
                    title: "提示",
                    content: "没有记录"
                });
                ...
                $("#newSiteTable").bootstrapTable("destroy");
                $("#newSiteTable").bootstrapTable({
                    url:"newSite/getNewSite",
                    method: "post",
                    striped: true,
                    dataType: "json",
                    pagination: true,
...
function exportAllSearch(){
    ...
    //E.start();
    $.post("newSite/exportAllSearch", params, function (data) {
        E.stop();
        if (data.result) {
...
function newSiteExport() {
    ...
    var E = Ladda.create(document.getElementById("newSiteExport"));
    E.start();
    $.post("newSite/exportNewSite", params, function (data) {
        E.stop();
        if (data.result) {
            fileDownload(data.fileName);
        } else {
...
function importFile() {
    ...
    $.ajaxFileUpload({
        ...
        success: function (data, status) {
            ...
            $.post("newSite/getFileContent", params, function (data) {
                E.stop();
                ...
                $.post("newSite/runTask", data, function (data) {
                    newSiteSearch();
                    layer.open({
                        title: "提示",
                        content: "kget解析入库成功",
