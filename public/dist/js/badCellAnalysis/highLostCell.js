...
function getAllCity(){
    ...
    var url = "highLostCell/getAllCity";
    $.ajax({
        type:"GET",
        url:url,
        dataType:"json",
...
function doSearchbadCell(table){
   var l = Ladda.create( document.getElementById( "search" ) );
    ...
    var X = Ladda.create( document.getElementById( "exportvolte" ) );
    l.start();
    E.start();
    X.start();
    ...
    $.get("highLostCell/templateQuery", params, function(data){
        var fieldArr=[];
        var fieldArr1=[];
        ...
        var badCellTable1 = $("#badCellTablevolte").grid({
            columns:fieldArr1,
            dataSource:newData1,
            pager: { limit: 10, sizes: [10, 20, 50, 100] },
            autoScroll:true,
        ...
        badCellTable1.on("rowSelect", function (e, $row, id, record) {
            $("#currentAlarmNum").removeClass();
            $("#less116ProportionNum").removeClass();
            $("#overlapCoverNum").removeClass();
            $("#less155ProportionNum").removeClass();
...
function reportvolte(record){
    ...
    if (weakcover > 20) {
        ...
        weakcoverpolar = (weakcover-2)*100/18; 
    }
    if(weakcover){
    ...
    $.ajax({
        type:"get",
        url:"highLostCell/highlostcellcanshu",
        data: params,
        dataType:"json",
        async : false,
        success:function(data){
        ...
        //data: data["data"],
            ...
            click: function(e) {
                if(e.point.category === "告警") {
                    ...
                    openWeakCoverCellModel(record.cell, record.city);
...
function report(record){
    ...
    if(record["最高RRC用户数"] == null || record["最高RRC用户数"] == 0 || record["最高RRC用户数"] == 0.00){
        ...
        $("#highTraffic").val(Math.round(record["最高RRC用户数"])+"个");                 
    }
    //$("#highTraffic").val(Math.round(record["最高RRC用户数"])+"个"); 
    ...
    //     $("#less116ProportionNum").addClass("glyphicon glyphicon-exclamation-sign");
    //     $("#less116ProportionNum").css("color","orange");
    //     if (record["Polar-邻区"] >6 && record["Polar-邻区"] < 12) {
    //         $("#needAddNeighNum").removeClass();
    //         $("#needAddNeighNum").addClass("glyphicon glyphicon-exclamation-sign");
    //         $("#needAddNeighNum").css("color","orange");
    //     } else if (record["Polar-邻区"] >= 12) {
    //         $("#needAddNeighNum").removeClass();
    //         $("#needAddNeighNum").addClass("glyphicon glyphicon-remove-sign");
    //         $("#needAddNeighNum").css("color","red");
    //     }
    ...
    if(record["Polar-最高RRC用户数"] == 0) {   //最高RRC用户数
       ...
       if(record["Polar-最高RRC用户数"] >300&&record["MAC层时延"]>100) {   //最高RRC用户数
          $("#highTrafficNum2").removeClass();
          // $("#highTrafficNum2").addClass("glyphicon glyphicon-ok-circle");
          // $("#highTrafficNum2").css("color","green");
...
function openAlarmModel(cell) {
    bsc_table_model("highLostCell/getCellAlarmClassifyTable", "#cellAlarmTable_model", {cell:cell,hours:null});  //当前告警
    bsc_table_model("highLostCell/getErbsAlarmClassifyTable", "#erbsAlarmTable_model", {cell:cell,hours:null});  //历史告警
...
function openNeighborCellModel(cell, city) {
    ...
    $.get("highLostCell/getLTENeighborData_model", paramsLTE_1, function(data){
        var fieldArr=[];
...
function openWeakCoverCellModel(cell, city) {
    ...
    var yAxis_name_left = "无线掉线率";
    var yAxis_name_right = "RSRP<-116的比例";
    var paramsLTE_1 = {
        cell : cell,
        city : city,
        yAxis_name_left : "无线掉线率",
        yAxis_name_right : "RSRP<-116的比例",
        table : "highLostCell_ex"
    };

    $.ajax({
        url:"highLostCell/getZhichaCell_chart",
        data:paramsLTE_1,
        type:"get",
        success:function(data){
    ...
    $.get("highLostCell/getWeakCoverCell_model", paramsLTE_1, function(data){
        var fieldArr=[];
        var text=data.content.split(",");
        var filename = data.filename;
        for(var i in data.rows[0]){       
...
function openzhichaCellModel(cell, city) {
    ...
    $.get("highLostCell/getzhichaCell_model", paramsLTE_1, function(data){
        var fieldArr=[];
        var text=data.content.split(",");
        var filename = data.filename;
        for(var i in data.rows[0]){       
...
function openOverlapCoverModel(cell, city) {
    ...
    $.get("highLostCell/getOverlapCover_model", paramsLTE_1, function(data){
        var fieldArr=[];
        var text=data.content.split(",");
        var filename = data.filename;
        for(var i in data.rows[0]){       
...
function openInterfereCellModel(cell, record) {
    ...
    $.ajax({
        url:"highLostCell/getGanraoCell_chart",
        data:paramsLTE_1,
        type:"get",
        success:function(data){
    ...
    $.get("highLostCell/getInterfereCell_model", paramsLTE_1, function(data){
        var fieldArr=[];
        var text=data.content.split(",");
        var filename = data.filename;
        for(var i in data.rows[0]){       
...
function getBaselineCheckData(cell,city){
    ...
    $.get("highLostCell/getBaselineCheckData", params,function(data){
...
function doSearchEvent_table(cell,city){
    ...
    $.get("highLostCell/getRrcResultTableData",params_rrc,function(data){
        if(data.error){
            return;
        }
        $("#rrcResultTable").grid("destroy", true, true);
    ...
    /*var grid = $("#rrcResultTable").grid({
        columns:fieldCol,
        dataSource:{ 
            url: "highLostCell/getRrcResultTableData", 
            success: function(data){
                if(data.error){
                   $("#rrcResultTable").grid("destroy", true, true);
                    return;
...
function rrcResult(cell,city){
    ...
    $.get("highLostCell/rrcResult", params_rrc,function(data){
...
function  getrrcResultDetail(params_rrc){
    ...
    var fieldArr=[]; 
    $.post("highLostCell/getRrcResultDetailTableField",params_rrc,function(data){
        $("#loading").html("");    
        E.stop();
        ...
        if (data.result == "error") {
            ...
            $(tableId).grid({
                columns:fieldArr,
                dataSource: { url: "highLostCell/getRrcResultDetailData",type:"post", data: params_rrc},
                params : params_rrc,
                pager: { limit: 10, sizes: [10, 20, 50, 100] },
                autoScroll:true,
                uiLibrary: "bootstrap",
...
function exportRrcDetail(){
    var E = Ladda.create( document.getElementById( "exportRrcDetail" ) );
    E.start();
    $.post("highLostCell/exportRrcResultDetail",params_temp,function(data){
        E.stop();
        if(data.result) {
...
function getRelevanceChart(record){
    var categories_ = [];
    var myData_ = [];
    if(record["无线掉线率_干扰"]>=record["无线掉线率_质差"]){
        if(record["无线掉线率_质差"]>=record["无线掉线率_弱覆盖"]){
            categories_ = ["无线掉线率_干扰","无线掉线率_质差","无线掉线率_弱覆盖"];
            myData_ = [parseFloat(record["无线掉线率_干扰"]),parseFloat(record["无线掉线率_质差"]),parseFloat(record["无线掉线率_弱覆盖"])];
        }else if(record["无线掉线率_干扰"]>=record["无线掉线率_弱覆盖"]){
            categories_ = ["无线掉线率_干扰","无线掉线率_弱覆盖","无线掉线率_质差"];
            myData_ = [parseFloat(record["无线掉线率_干扰"]),parseFloat(record["无线掉线率_弱覆盖"]),parseFloat(record["无线掉线率_质差"])];
        }else{
            categories_ = ["无线掉线率_弱覆盖","无线掉线率_干扰","无线掉线率_质差"];
            myData_ = [parseFloat(record["无线掉线率_弱覆盖"]),parseFloat(record["无线掉线率_干扰"]),parseFloat(record["无线掉线率_质差"])];
        }
    }else{
        if(record["无线掉线率_干扰"]>=record["无线掉线率_弱覆盖"]){
            categories_ = ["无线掉线率_质差","无线掉线率_干扰","无线掉线率_弱覆盖"];
            myData_ = [parseFloat(record["无线掉线率_质差"]),parseFloat(record["无线掉线率_干扰"]),parseFloat(record["无线掉线率_弱覆盖"])];
        }else if(record["无线掉线率_质差"]<=record["无线掉线率_弱覆盖"]){
            categories_ = ["无线掉线率_弱覆盖","无线掉线率_质差","无线掉线率_干扰"];
            myData_ = [parseFloat(record["无线掉线率_弱覆盖"]),parseFloat(record["无线掉线率_质差"]),parseFloat(record["无线掉线率_干扰"])];
        }else{
             categories_ = ["无线掉线率_质差","无线掉线率_弱覆盖","无线掉线率_干扰"];
            myData_ = [parseFloat(record["无线掉线率_质差"]),parseFloat(record["无线掉线率_弱覆盖"]),parseFloat(record["无线掉线率_干扰"])];
        }
    }
...
var drawMapOut = function (t, scell) {
   ...
   // console.log(scell);
    if(t == "最大RRC连接用户数"){
        url = "highLostCell/RRCusers";
    }else if(t=="无线掉线率"){
        url = "highLostCell/wireLessLost";
    }else if(t=="PUSCH上行干扰电平"){
        url = "highLostCell/PUSCHInterfere";
    }else if(t=="origin"){
        url = "highLostCell/switchData";
    }
                    ...
                    events: {
                        click: function(e, data) {
                           // console.log("click",e,data);
                            ...
                            $.get("highLostCell/switchDetail", params, function(data){
...
var drawMapIn = function (t, scell) {
    ...
    switchOutTableIn();
    if(t == "最大RRC连接用户数"){
        url = "highLostCell/RRCusersin";
    }else if(t=="无线掉线率"){
        url = "highLostCell/wireLessLostin";
    }else if(t=="PUSCH上行干扰电平"){
        url = "highLostCell/PUSCHInterferein";
    }else if(t=="origin"){
        url = "highLostCell/handoverin";
    }
    var S = Ladda.create( document.getElementById( "search" ) );
    var E = Ladda.create( document.getElementById( "export" ) );
    S.start();E.start();
                ...
                drawOptions: { // 绘制参数
                    ...
                    events: {
                        click: function(e, data) {
                            ...
                            $.get("highLostCell/handOverInDetail", params, function(data){
                                var newData = JSON.parse(data).data;
                                console.log(newData);
                                $("#bMapTable").grid("destroy", true, true);
                                $("#bMapTable").grid({
...
function switchOutTable(){
    ...
    $.get("highLostCell/switchOutTable", params, function(data){
...
function switchOutTableIn(){
    ...
    $.get("highLostCell/switchOutTableIn", params, function(data){
        var myData = eval(data);
        //最大RRC连接用户数
        $("#nummore200").html(myData[0]);
        $("#num100to200").html(myData[1]);
...
function getIndexChartData(record){
    ...
    $.get("highLostCell/getIndexChartData",params1,function(data){ 
        $("#chart_zhaozi").hide(); 
...
function getIndexTableData(record){
    ...
    $.get("highLostCell/getIndexTableData", params2,function(data){
        var fieldArr=[];
        var text=(JSON.parse(data).content).split(",");
        var filename = JSON.parse(data).filename;
...
function storage() {
 ...
 //        }
    // });
    $.post("highLostCell/storage", params, function (data) {
        if (data) {
            path = data;
            if (type == "ctr") {
                $("#ctrTypeDiv").show();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (returnData.status == "true") {
                ...
                $.post("highLostCell/deleteAutoDir",{"tracePath":tracePath});
...
function initTable() {
    ...
    $(table).treegrid({
        ...
        columns: [[
            ...
            {field: "无线掉线次数", title: "无线掉线次数", width: 100},
        ]]
    });
}
...
function getCTRJump(record) {
    ...
    var losttimetoday = record["无线掉线次数(今日)"];
    var city = record.city;
    var cell = record.cell;
    var type = "ctr";
    ...
    $.post("highLostCell/ctrTreeItems", params, function (data) {
        S.stop();
        // var returnData = JSON.parse(data);
        $("#ctrData").html("");
        $("#ctrData").append(data);
...
function getNeighborCellMapData(params){
    $.post("highLostCell/getNeighborCellMapData", params, function(data){
        drawMap(data);
    });
}
...
function getHighInterfereHandlePrinciple(record) {
    ...
    $.post("highLostCell/getTreatmentPrinciple", params, function(data){
        // console.log(data);
        if(data["flag"] == false) {
        ...
        if(data["pZeroNominalPUCCH"] != "-96") {
            pZeroNominalPUCCH = "pZeroNominalPUCCH建议从"+data["pZeroNominalPUCCH"]+"修改为-96;";
        }else {
            pZeroNominalPUCCH = "pZeroNominalPUCCH当前值为-96;";
        }
        if(data["alpha"] != "1") {
            alpha = "alpha建议从"+data["alpha"]+"修改为1;";
        }else {
            alpha = "alpha当前值为1;";
        }
