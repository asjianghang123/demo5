...
util.extend(DataRange.prototype, {
    ...
    draw: function draw() {
        ...
        }else if (this.get("layer").getDrawType() === 'density') {
            this.get("layer").dataRangeControl.drawChoroplethSplit(this.splitList, this.get('drawOptions'));
        } else {
            this.get("layer").dataRangeControl.hide();
        }
...
MapEvent.prototype.removeEvent = function (type,id) {
...
util.extend(Layer.prototype, {
    ...
    hide: function hide() {
        ...
        mapv.mapEvent.removeEvent("click",0);
...
util.extend(DataRangeControl.prototype, {
    ...
    drawChoroplethSplit: function drawChoroplethSplit(splitList, drawOptions) {
        var canvas = this.canvas;
        if(splitList[0].start == 'chinese'){ //如果start是chinese,则改变显示方式
            canvas.width = 90;
            canvas.height = 70;
            ...
            for (var i = 1; i < splitList.length; i++) {
                ctx.beginPath();
                ctx.arc(15, (i-1) * 25 + 15, 5, 0, Math.PI * 2, false);
                var text = (splitList[i].end);
                ctx.closePath();
                ctx.fillStyle = splitList[i].color;
                ctx.fill();
                ctx.fillStyle = '#333';
                ctx.fillText(text, 25, (i-1) * 25 + 20);
            };
        }else{
            canvas.width = 100;
            canvas.height = 190;
...
DrawScale.prototype.initialize = function (map) {
    ...
    //genius隐藏图例20160919-li-start
    box.style.width = 0 + 'px';
    box.style.height = 0 + 'px';
    //genius隐藏图例20160919-li-end
...
ChoroplethDrawer.prototype.init = function () {
    var self = this;
    var mapv = this.getMapv();
    ...
    function addEventFn(i) {
        mapv.mapEvent.addEvent(i, function (e) {
            var data = self.getLayer().getData();
            ...
            for (var j = 0, len = data.length; j < len; j++) {
                ...
                {
                    case 'D':
                        if (Math.pow(e.offsetX - item.px, 2) + Math.pow(e.offsetY - item.py, 2) > Math.pow(radius, 2)) {
                            continue;
                        } else {
                            var dir = (270 + parseInt(item.dir)) % 360;
                            var aX = radius * Math.cos(((dir + 30) / 360) * 2 * Math.PI);
                            var aY = radius * Math.sin(((dir + 30) / 360) * 2 * Math.PI);
                            var bX = e.offsetX - item.px;
                            var bY = e.offsetY - item.py;

                            var length = Math.sqrt(Math.pow(bX, 2) + Math.pow(bY, 2)) * radius;
                            var alpha = Math.acos((aX * bX + aY * bY) / length);

                            if (alpha < (30 / 360) * 2 * Math.PI) {
                                sdata.push(item);
                            }
                        }
                        break;
                    case 'F':
                        if (Math.pow(e.offsetX - item.px, 2) + Math.pow(e.offsetY - item.py, 2) > Math.pow(radius + 10, 2)) {
                            continue;
                        } else {
                            var dir = (270 + parseInt(item.dir)) % 360;
                            var aX = radius * Math.cos(((dir + 30) / 360) * 2 * Math.PI);
            ...
            if(sdata.length != 0) {
                options.events[i](e, sdata);
            }
        });
    }
}

ChoroplethDrawer.prototype.isPointInCircularSector = function () {
    ...
    if (dataType === 'polyline' || dataType === 'polygon') {
        ...
        if(type == 'switch'){  //只显示连线
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                ...
                var dir = (270+parseInt(item.dir))%360;
                if(item.band == 'D') {
                    ctx.arc(item.px, item.py, radius, (dir/360)*2*Math.PI, ((dir + 60)/360)*2*Math.PI);
                }else if(item.band == 'F') {
                    ctx.arc(item.px, item.py, radius + 10, ((dir + 20)/360)*2*Math.PI, ((dir + 40)/360)*2*Math.PI);
                }else if(item.band == 'E') {
                    ctx.arc(item.px, item.py, radius - 10, 0, 2*Math.PI);
                }
                ...
                if(item.lineCount=='null'){ //不连线
                    continue;
                }
                ctx.beginPath();
                ctx.moveTo(master.px, master.py);
                ctx.lineTo(item.px,item.py);
                if(item.lineCount=='red'){   //设置线的颜色
                    ctx.strokeStyle = 'red';
                }else if(item.lineCount=='blue'){
                    ctx.strokeStyle = 'blue';
                }else if(item.lineCount=='yellow'){
        ...
        if(type == 'switchout') {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                var item = data[i];
                ctx.lineWidth = item.lineWidth ? item.lineWidth : 1;
                ctx.fillStyle = this.dataRange.getColorByRange(item.count);
                ctx.beginPath();
                ctx.moveTo(item.px, item.py);
                var dir = (270+parseInt(item.dir))%360;
                ...
                ctx.fillStyle = 'black'; //这是箭头的颜色
                
                if(item.lineCount=='red'){ //设置箭头颜色
                    ctx.fillStyle = 'red';
                }else if(item.lineCount=='blue'){
                    ctx.fillStyle = 'blue';
                }else if(item.lineCount=='yellow'){
                ...
                var ang=(item.px-master.px)/(item.py-master.py);
                ang=Math.atan(ang);
                if(item.py-master.py >= 0){
                    ctx.rotate(-ang);
                }else{
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                ...
                ctx.beginPath();
                ctx.moveTo(item.px, item.py);
                ctx.lineTo(master.px,master.py);
                if(item.lineCount=='red'){   //设置线的颜色
                    ctx.strokeStyle = 'red';
                }else if(item.lineCount=='blue'){
                    ctx.strokeStyle = 'blue';
                ...
                //画箭头
                ctx.translate(master.px,master.py);
                //我的箭头本垂直向下，算出直线偏离Y的角，然后旋转 ,rotate是顺时针旋转的，所以加个负号
                var ang=(master.px-item.px)/(master.py-item.py);
                ang=Math.atan(ang);
                if(master.py-item.py >= 0){
                    ctx.rotate(-ang);
                }else{
                    ctx.rotate(Math.PI-ang);//加个180度，反过来
                }
...
function recGrids(obj, map) {
    ...
    for (var i = 0; i < stockXA.length; i++) {
        for (var j = 0; j < stockYA.length; j++) {
            var name = stockXA[i] + '_' + stockYA[j];
            grids[name] = {count:0,val:0};
        }
    }

    //搜索半径
    var radius = 2000 / zoomUnit;
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        for (var j = 0; j < stockXA.length; j++) {
            var dataX = Number(stockXA[j]);
            var centerX = dataX + gridStep / 2;
            if (x + radius >= centerX && x - radius < centerX) {
                for (var k = 0; k < stockYA.length; k++) {
                    var dataY = Number(stockYA[k]);
                    var centerY = dataY + gridStep /2;
                    if (Math.pow(centerX - x,2) + Math.pow(centerY - y,2) < Math.pow(radius,2)) {
                        grids[stockXA[j] + '_' + stockYA[k]].val += val;
                        grids[stockXA[j] + '_' + stockYA[k]].count += 1;
                        //val = grids[stockXA[j] + '_' + stockYA[k]].val;
                    }
                }
            }
        }
...
function drawRec(obj) {
    ...
    for (var i in grids) {
        ...
        var x = sp[0];
        var y = sp[1];
        var v = (grids[i].val - min) / step;
        //var color = fillColors[v | 0];
        var color = obj.dataRange.getColorByRange(grids[i].val / grids[i].count);
        try {
            if (options.opacity) {
        ...
        if (grids[i].val === 0 || isTooSmall || isTooBig) {
        ...
        ctx.strokeRect(x, y, gridStep, gridStep);
...
function formatParam() {
    ...
    if (/px$/.test(size)) {
        ...
        var px = parseInt((parseInt(size, 10) / this.zoomUnit),10);
        if(px  < 5) {
            size = 5;
        }else {
            size = parseInt((parseInt(size, 10) / this.zoomUnit),10);
        }
        size = size * this.zoomUnit;
    }
    options.size = size;
