...
use DateInterval;
use Config;
use App\Models\NEWSITESTATE\NewSite;
 ...
 * @link     https://laravel.com/docs/5.4/controllers
 */
class NewSiteController extends MyRedis
{
    /**
     * 获得城市列表
     *
    ...
    public function getTableField()
    {
        ...
        if ($filter != "") {
            ...
            $row = NewSite::whereRaw($filter)->first();
        }else{
            $row = NewSite::first();
        }
        $data = array();
    ...
    {   
        ...
        if ($filter != "") {
            $filter = substr($filter, 4);
            $rows = NewSite::whereRaw($filter)->orderBy('datetime_id','desc')->paginate($page)->toArray();
        }else{
            $rows = NewSite::orderBy('datetime_id','desc')->paginate($page)->toArray();
        }
        $items = array();
    ...
    public function exportAllSearch()
    {
        ...
        if ($filter != "") {
            $filter = substr($filter, 4);
            $items = NewSite::whereRaw($filter)->orderBy('datetime_id','desc')->get()->toArray();
        }else{
            $items = NewSite::orderBy('datetime_id','desc')->get()->toArray();
        }
        $keys = array();
    ...
    public function exportNewSite()
    {
        ...
        if ($filter != "") {
            $filter = substr($filter, 4);
            $items = NewSite::select('datetime_id','city','ecgi','tac','ipAddr')->whereRaw($filter)->where('IsExistKget','N')->whereRaw(" (ipAddr = '' or ipAddr is null or IsGetKget!='Y') ")->orderBy('datetime_id','desc')->get()->toArray();
        }else{
            $items = NewSite::select('datetime_id','city','ecgi','tac','ipAddr')->where('IsExistKget','N')->whereRaw(" (ipAddr = '' or ipAddr is null or IsGetKget!='Y') ")->orderBy('datetime_id','desc')->get()->toArray();
        }
        $keys = array();
        foreach (array_keys($items[0]) as $key) {
            array_push($keys, trans("message.newSite.".$key));
     ...
     * @return mixed
     */
    public function getFileContentBak()
    {
        $dbc = new DataBaseConnection();
        ...
        for ($i=1; $i < $len_result; $i++) { 
            ...
            $tac = $result[$i][3];
            $ipAddr = $result[$i][4];
            NewSite::where('datetime_id', $datetime_id)->where('city', $city)->where('ecgi', $ecgi)->where('tac', $tac)->update(array('ipAddr' => $ipAddr));
            if ($ipAddr != '') {
                ...
                //sh siteGetKget.sh 10.40.51.186 root ericsson 100.105.154.30 2018052516 kget180514
        ...
        foreach ($ipList as $city => $content) {
            $fileName = "/data/trace/siteKget/ipList/ipList_newSite_".$city."_".$this->udate('YmdHisu').".txt";
    ...
    public function getFileContent()
    {
        ...
        for ($i=1; $i < $len_result; $i++) { 
            ...
            if ($ipAddr != '') {
                NewSite::where('datetime_id', $datetime_id)->where('city', $city)->where('ecgi', $ecgi)->where('tac', $tac)->whereRaw(" IsGetKget != 'Y' ")->update(array('ipAddr' => $ipAddr));
                $count = NewSite::where('datetime_id', $datetime_id)->where('city', $city)->where('ecgi', $ecgi)->where('tac', $tac)->where('IsExistKget','N')->whereRaw(" IsGetKget != 'Y' ")->count();
                if ($count > 0){
                    if (array_key_exists($city, $ipList)) {
    ...
    public function runTask(){
        ...
        //$len_result = count($result);
        if ($ipList) {
            include_once("/opt/lampp/htdocs/genius/app/Http/Controllers/SpecialFunction/SiteKget.php");
            $siteKget = new \SiteKget();
