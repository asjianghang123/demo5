    ...
    public function sendInfo2Docker($ipListOri, $info = null, $source = null){
        ...
        foreach ($ipListOri as $city => $ipAddrs) {
            foreach ($ipAddrs as $ipAddr) {
                ...
                $sql = "select count(*) from $tableName where IsExistKget='N' and IsGetKget !='Y' and city='$city' and ipAddr ='$ipAddr'";
        ...
        foreach ($ipList as $city => $ipAddrs) {
            ...
            $command = "sh /opt/gback/gtools/loginSiteGetKget/runScpExpect.sh $fileName $remoteIp $port $remoteFile >> /data/trace/siteKget/log/runScpExpect.log";
            //print_r($command);
            exec($command);
        }
        //新站检查结果上传到阿里云
