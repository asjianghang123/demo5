...
/**
 * OverlappingDonorAnalysisController.php
 *
...
/**
 * 重叠覆盖施主分析
 * Class OverlappingDonorAnalysisController
 *
 * @category Controllers
 * @package  App\Http\Controllers
 * @author   ericsson <genius@ericsson.com>
 ...
 * @link     https://laravel.com/docs/5.4/controllers
 */
class OverlappingDonorAnalysisController extends MyRedis
{
    ...
    public function getData()
    {
        ...
        $items     = array();
        $ecgi = substr($ecgi, 5);
        $row = MroInterfereMatrix_day::on($dataBase)->where('dateId', $date)->where('ecgi', $ecgi)->orderBy('intensity', 'desc')->get()->toArray();
        $tableData = array();
        foreach ($row as $r) {
            array_push($items,"'".$r["ecgiNc"]."'");
            array_push($tableData, $r);
        }

        $cells = array();
        if (count($items) > 0) {
            $result['targetCell'] = SiteLte::whereRaw("substring_index(ecgi,'-',-2)='$ecgi'")->first()->toArray();
    ...
    public function getDetailData()
    {
        ...
        foreach ($cells as $cell) {
            ...
            $rs  = $db1->query($sql);
            $row = $rs->fetchAll(PDO::FETCH_NUM);
            array_push($ecgiNcs, $row[0][0]);
        }

        $items = array();
        foreach ($ecgiNcs as $ecgiNc) {
    ...
    public function getDataGroupByDate()
    {
        ...
        $this->type = $dbname.':overlappingDonorAnalysis';
    ...
    public function exportData()
    {
        ...
        $filename = "common/files/重叠覆盖施主分析_" . $date .'_'.$cellName. ".csv";
