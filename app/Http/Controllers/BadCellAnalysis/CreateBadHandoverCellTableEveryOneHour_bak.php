<?php
    ...
    $sql = "SELECT cell FROM Temp_BadHandoverCellTableEveryOneHour;";
    ...
    foreach ($rows as $key => $value) {
        ...
        $num = count($row);
        if ($num == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `告警数量`=0 WHERE cell='$cell'");    
        } else {
        ...
        getBadHandover_prepare($cell, $date, $hour, $pdo);                             //相关性--切换成功率_准备切换成功率
        getBadHandover_exec($cell, $date, $hour, $pdo);                                //相关性--切换成功率_执行切换成功率
        geBadHandover_interfere($cell, $date, $hour, $pdo);                              //相关性--切换成功率&干扰
        getBadHandover_zhicha($cell, $date, $hour, $pdo);                                  //相关性--切换成功率&质差
    ...
    {    
      ...
      $rows = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
      $num = $rows[0]['num'];
      $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `告警数量`=$num WHERE cell='$cell'");    
    }
    //弱覆盖--RSRP<-116的比例
    function getNumOfDiagnosisDataFilter_weakCover($cell, $date, $hour, $pdo) 
    {
      $sql = "select AVG(`RSRP<-116的比例`) as NUM from badHandoverCell_ex where day_id >= '$date' AND cell='$cell' AND hour_id='$hour';";
      $row = $pdo->query($sql, PDO::FETCH_ASSOC)->fetchall();
      $num = $row[0]['NUM'];
      if ($num === null) {
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `RSRP<-116的比例`=0 WHERE cell='$cell'");
      } else {
    ...
    {
      $sql = "SELECT SUM(`最大RRC连接用户数`)/20 AS NUM FROM badHandoverCell_ex WHERE cell = '$cell' AND day_id = '$date' AND hour_id='$hour';";
      $row = $pdo->query($sql, PDO::FETCH_ASSOC)->fetchall();
      $num = $row[0]['NUM'];
      if ($num === null) {
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `关联度`=0 WHERE cell='$cell'");
      } else {
       ...
       function getNumOfDiagnosisDataFilter_overlapCover($city, $cell, $date, $hour, $pdo)
       {
        ...
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `重叠覆盖度`=0 WHERE cell='$cell'");
       ...
       function getNumOfDiagnosisDataMR($city, $cell, $date, $hour, $pdo)
       {
        ...
        if (count($row) == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `需要加邻区数量`=0 WHERE cell='$cell'");
        } else {
            $ecgi = $row[0]['ecgi'];
       ...
       {
           $sql = "select AVG(`RSRQ<-15.5的比例`) as NUM from badHandoverCell_ex where day_id = '$date' AND hour_id = '$hour' AND cell='$cell'";
           $row = $pdo->query($sql, PDO::FETCH_ASSOC)->fetchall();
           $num = $row[0]['NUM'];
           if ($num === null) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `RSRQ<-15.5的比例`=0 WHERE cell='$cell'");
        } else {
       ...
       function getNumOfDiagnosisDataFilter_parameter($city, $cell, $date, $hour, $pdo)
       {
        ...
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `参数`=$value WHERE cell='$cell'");
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-参数`=$value WHERE cell='$cell'");
       }
       //极地图
       function getPolarMapData($city, $cell, $date, $hour, $pdo) 
       {
           ...
           if (count($row)==0) {
               $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-告警`=0 WHERE cell='$cell'");
           } else {
               $erbs = $row[0]['siteName'];
            ...
            if (count($row) == 0) {
                ...
                if ($row[0]['num'] > 100) {
                    $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-告警`=100 WHERE cell='$cell'");
                } else {
                    $num = intval($row[0]['num']);
                    $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-告警`=$num WHERE cell='$cell'");
                }
            }
           }
        
        //弱覆盖
        $sql = "SELECT AVG(`RSRP<-116的比例`) AS num FROM badHandoverCell_ex WHERE day_id = '$date' AND hour_id='$hour' AND cell = '$cell';";
        $row = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        $weakCoverRateFlag = '';             
        if (count($row) == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-弱覆盖`=0 WHERE cell='$cell'");
            $weakCoverRateFlag = 0;
        } else {
            if ($row[0]['num'] > 20) {
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-弱覆盖`=100 WHERE cell='$cell'");
            } else if ($row[0]['num'] < 2) {
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-弱覆盖`=0 WHERE cell='$cell'");
            } else {
                ...
                if ($num > 100) {
                    ...
                    $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-弱覆盖`=$num WHERE cell='$cell'");
                }               
            }
            if ($row[0]['num'] > 11) {
                $weakCoverRateFlag = 1;
        ...
        if (count($row) == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-重叠覆盖`=0 WHERE cell='$cell'");
            $overCoverFlag = 0;
        } else {
            if ($row[0]['num'] < 0.2) {
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-重叠覆盖`=0 WHERE cell='$cell'");
                ...
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-重叠覆盖`=$num WHERE cell='$cell'");
            } else {
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-重叠覆盖`=100 WHERE cell='$cell'");
            }
            if ($row[0]['num'] > 5) {
                $weakCoverRateFlag = 1;
            } else {
        ...
        $row = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        if (count($row) == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-质差`=0 WHERE cell='$cell'");
        } else {
            if ($row[0]['num'] == 0) {
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-质差`=0 WHERE cell='$cell'");
            } else {
                if ($weakCoverRateFlag == 0) {
                    if ($overCoverFlag == 0) {
                        $point = $row[0]['num']*5;
                        if ($point >100) {
                            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-质差`=100 WHERE cell='$cell'");
                        } else {
                            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-质差`=$point WHERE cell='$cell'");
                        }        
                    } else {
        ...
        $row = $res->fetchAll(PDO::FETCH_ASSOC);
        if (count($row) == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-邻区`=0 WHERE cell='$cell'");
        } else {
            $ecgi = $row[0]['ecgi'];
            $db = getConn($city);
            $table = 'mreServeNeigh_day';
            ...
            if (count($row) == 0) {
                ...
                if ($row[0]['num'] == 0) {
                    ...
                    if ($weakCoverRateFlag == 1) {    //非弱覆盖小区
                        ...
                        if ($num > 100) {
                            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-邻区`=100 WHERE cell='$cell'");
                        } else {
                            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-邻区`=$num WHERE cell='$cell'");
                        }                   
        ...
        $avg = 0;
        if (count($row) == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-干扰`=0 WHERE cell='$cell'");
        } else {
            foreach ($row[0] as $val) {
                if ($val == null) {
                    continue;
            ...
            if ($i == 0) {
                ...
                $avg = $avg / $i;
                if ($avg >= -102) {
                    $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-干扰`=100 WHERE cell='$cell'");
                } else if ($avg <= -112) {
                    $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-干扰`=0 WHERE cell='$cell'");
                } else {
                    $point = ($avg + 112)*10;
                    if ($point > 100) {
                        ...
                        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-干扰`=$point WHERE cell='$cell'");
                    }
                }
            }
        }
        //高话务
        $sql = "SELECT SUM(`最大RRC连接用户数`)/20 AS num FROM badHandoverCell_ex WHERE cell = '$cell' AND day_id = '$date';";
        $row = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        if (count($row) == 0) {
            $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-高话务`=0 WHERE cell='$cell'");
        } else {
            if ($row[0]['num'] > 100) {
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-高话务`=100 WHERE cell='$cell'");
            } else {
                $point = intval($row[0]['num']);
                $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-高话务`=$point WHERE cell='$cell'");
            }
        }
        //参数
        ...
        // $erbs = '';
        // if (count($row) == 0) {
        //     $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-参数`=0 WHERE cell='$cell'");
        // }else {
        //     $erbs = $row[0]['siteName'];
        // }
        // $db = getParamConn($dbname);
        ...
        //         $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-参数`=100 WHERE cell='$cell'");
        //     } else {
        ...
        //                 $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-参数`=0 WHERE cell='$cell'");
        //             }else {
        //                 $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `Polar-参数`=50 WHERE cell='$cell'");
        //             }
        //         }    
        //     }
        // }
    ...
    //相关性--切换成功率_准备切换成功率
    function getBadHandover_prepare($cell, $date, $hour, $pdo)  
    {
        $startTime = date('Y-m-d', strtotime("-1 day")); 
        $endTime = date('Y-m-d');
        $value_x = [];
        ...
        while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
            ...
            array_push($value_y, $row['准备切换成功率']);
        }
        $data['data'] = getRelevanceData($value_x, $value_y);
        $data['date_from'] = $startTime;
        $dataStr = implode(',', $data);
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `切换成功率_准备切换成功率`='$dataStr' WHERE cell='$cell'");
    }              
    //相关性--切换成功率_执行切换成功率            
    function getBadHandover_exec($cell, $date, $hour, $pdo)
    {
        $startTime = date('Y-m-d', strtotime("-1 day")); 
        $endTime = date('Y-m-d');
        $value_x = [];
        ...
        while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
            array_push($value_x, $row['切换成功率']);
            array_push($value_y, $row['执行切换成功率']);
        }
        $data['data'] = getRelevanceData($value_x, $value_y);
        $data['date_from'] = $startTime;
        $dataStr = implode(',', $data);
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `切换成功率_执行切换成功率`='$dataStr' WHERE cell='$cell'");
    } 
    //相关性--切换成功率&干扰  
    function geBadHandover_interfere($cell, $date, $hour, $pdo)
    {
        $startTime = date('Y-m-d', strtotime("-1 day")); 
        $endTime = date('Y-m-d');
        $value_x = [];
        ...
        $data['date_from'] = $startTime;
        $dataStr = implode(',', $data);
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `切换成功率_干扰`='$dataStr' WHERE cell='$cell'");
    }    
    //相关性--切换成功率&质差    
    function getBadHandover_zhicha($cell, $date, $hour, $pdo)
    {
        $startTime = date('Y-m-d', strtotime("-1 day")); 
        $endTime = date('Y-m-d');
        $value_x = [];
        ...
        $data['date_from'] = $startTime;
        $dataStr = implode(',', $data);
        $pdo->query("UPDATE Temp_BadHandoverCellTableEveryOneHour SET `切换成功率_质差`='$dataStr' WHERE cell='$cell'");
    }                                             
