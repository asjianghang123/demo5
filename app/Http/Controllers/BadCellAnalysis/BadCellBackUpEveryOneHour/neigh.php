<?php
    ...
    $sql = "CREATE TABLE IF NOT EXISTS `temp_neigh` (
    ...
    $dbA = $db = new PDO("mysql:host=$host;dbname=$database", "$username", "$password");
    $dbA = $db = new PDO("mysql:host=localhost;dbname=AutoKPI", 'root', 'mongs');        
    $sql = "select count(distinct city) as num from temp_neigh";
    $res=$dbA->query($sql);
    $row=$res->fetch();
    // print_r($row);
    // return;
    if ($row&&$row['num']>=5) {
        $sql = "TRUNCATE TABLE temp_neigh;";
        $dbA->query($sql);
    }                
    // $dbA = $db = new PDO("mysql:host=localhost;dbname=AutoKPI", 'root', 'mongs');
    // $dbA->query($sql);
    ...
    foreach ($cityArr as $dbServer) {
        $sql = "SELECT * FROM mongs.databaseconn WHERE connName='$dbServer'";
 ...
 //    $dbA->query($sql);
 //    $sql = "INSERT INTO NeighBadHandoverCellTableEveryOneHour SELECT * FROM temp_neigh;";
 //    $dbA->query($sql);
    ...
    function queryTemplate($city, $pmDB, $dbA, $subNets, $date, $hour, $cell)
    {
        ...
        // print_r($strs);
        $sql = "INSERT INTO temp_neigh VALUES $strs;";
        // print_r($sql);
        $dbA->query($sql);
    }
    ...
    {
        $SQL = "select subNetwork from mongs.databaseconn where connName = '$city'";
        $res = $db->query($SQL);
