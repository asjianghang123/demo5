<?php
    ...
    //1-1.建表
    $file = "sql/createTable_lowAccessCell.sql";
    if (is_file($file)) {
        $sql = file_get_contents($file);
    } else {
        echo "createTable_lowAccessCell.sql 不存在,请新建表!";
        return;
    }
    $pdo->query($sql);
    //1-2运行sql，开始插入初始数据
    $file = "sql/insert30Data_lowAccessCell.sql";
    if (is_file($file)) {
        $sql = str_replace('$city', $city, file_get_contents($file));
    } else {
        echo "insert30Data_lowAccessCell.sql 不存在,请新建表!";
        return;
    }
    $pdo->query($sql); 

    //2.获取所有小区名=基站名 一一对应
    $sql = "SELECT cell,siteName FROM temp_lowaccesscell,mongs.siteLte WHERE temp_lowaccesscell.city = ? AND cell=cellName;";
    $sth = $pdo->prepare($sql);
