...
/**
 * SlotOnlineTimeAnalysisController.php
 *
...
/**
 * 板卡在网时间分析
 * Class SlotOnlineTimeAnalysisController
 *
 * @category AlarmAnalysis
 * @package  App\Http\Controllers\AlarmAnalysis
 * @author   ericsson <genius@ericsson.com>
 ...
 * @link     https://laravel.com/docs/5.4/controllers
 */
class SlotOnlineTimeAnalysisController extends Controller
{
    /**
     * 获得城市列表
     *
    ...
    public function getAllSlot()
    {
        ...
        $rows = SlotSerialNumberRecord::whereIn('subNetwork', $subNetworkArr)->paginate($limit)->toArray();
    ...
    public function exportAllSlot()
    {
        $cities = Input::get('city');
        $subNetworkArr = $this->getSubNetworkArr($cities);
        $rows = SlotSerialNumberRecord::whereIn('subNetwork', $subNetworkArr)->get()->toArray();
        $result = array();
    ...
    public function getDisappearSlot()
    {
        ...
        $displayLength = Input::get('limit', 10);
        ...
        $subNetworkArr = $this->getSubNetworkArr($cities);
        $total = SlotSerialNumberRecord::whereNull('是否在网')->whereIn('subNetwork', $subNetworkArr)->count();
        $rows = DB::select("select * from slotSerialNumberRecord where `是否在网` is null and subNetwork in (" . $subNetwork . ") " . $limit);
        $result = array();
        if (count($rows) > 0) {
    ...
    function exportDisappearSlot()
    {
        ...
        $subNetwork = substr($subNetwork, 0, -1);
        $rows = DB::select("select * from slotSerialNumberRecord where `是否在网` is null and subNetwork in (" . $subNetwork . ")");
        $result = array();
    ...
    public function getOneSlotInfo()
    {
        ...
        $limit = Input::get('limit', 10);
        $serialNumber = Input::get('serialNumber');
        $rows = SlotSerialNumberRecordTotal::orderBy('kgetTime', 'asc')->where('productData_serialNumber', $serialNumber)->paginate($limit, array('*'), 'page', $page)->toArray();
        $result = array();
        if ($rows['total'] > 0) {
            $keys = array_keys($rows['data'][0]);
            $titles = array();
