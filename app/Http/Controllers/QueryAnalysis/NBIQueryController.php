 ...
 * @link     https://laravel.com/docs/5.4/controllers
 */
class NBIQueryController extends GetTreeData
{
        ...
        if ($login_user === 'admin') {
            if ($format == "TDD") {
                foreach ($users as $user) {
                    $userStr       = $user->user;
                    $templateNames = DB::table('templateNbi')->where('format', '=', $format)->where('templateName', 'like', $inputData)->where('user', '=', $userStr)->get();
    ...
    public function getTreeData()
    {
        ...
        if ($login_user === 'admin') {
            if($format == "TDD"){
                foreach ($users as $user) {
                    $userStr       = $user->user;
                    $templateNames = DB::table('templateNbi')->where('format', '=', $format)->where('user', '=', $userStr)->get();
                    foreach ($templateNames as $templateName) {
                        array_push($arrUser, array("text" => $templateName->templateName, "id" => $templateName->id));
                    }
    ...
    public function templateQuery()
    {
        ...
        $resultText     = $getItems->getResultText($locationDim, $timeDim, $format);
        $result['text'] = $resultText.','.$nbiKpis['names'];
        // var_dump($resultText);return;
        $useKpis        = "";
        ...
        // dd('韶关');
        // dd($city);
        // $city = mb_convert_encoding($city, 'utf-8', 'gb2312');print_r($city);
        ...
        // return;
        $newArr = $nbiQuery->parseNbiKpiTest($nbiKpis);
        if ( count($newArr['judge']) > 0 ) {
            return json_encode(array('check'=>'Check:'.implode(',', $newArr['judge'])));
        }
        $nbiFormulaFilter = $newArr['nbiKpiFormula'];
        ...
        foreach ($citys as $city) {
            if ($city == '无锡' || $city == '苏州' || $city == '南通' || $city == '镇江' || $city == '常州') {
                if ($format=='TDD') {
                    ...
                    $select = $nbiQuery->getSelectTDDNbm($locationDim, $timeDim, $selectNbiLocations);
                } else if ($format=='FDD') {
                    $selectEutranCellFdd = $nbiQuery->getSelectEutranCellFddNbm($locationDim, $timeDim, $nbiFormulaFilter, $hour, $minute, $cell);
                    $ThisDate            = '';
                    for ($i = strtotime($startTime); $i <= strtotime($endTime); $i += 86400) {
                        $ThisDate = $ThisDate.'\''.date("Y-m-d", $i).'\',';
                    }
                    ...
                    $ThisDate = substr($ThisDate, 0, (strlen($ThisDate) - 1));
                    // 筛选项：日期
                    $selectDateId = $selectEutranCellFdd." where date_id in ($ThisDate)";
                    // 筛选项：时间维度
                    $selectTimeDim = $nbiQuery->getSelectTimeDimNbm($timeDim, $hour, $minute, $selectDateId);
                    // 筛选项：区域维度
                    $nbiLocation       = $nbiQuery->getLocationNbm($city);
                    ...
                    // 筛选项：基站or小区
                    $selectNbiLocations = $nbiQuery->getSelectNbiLocationNbm($selectNbiLocation, $locationDim, $erbs, $cell);
                    $select = $nbiQuery->getSelectFDDNbm($locationDim, $timeDim, $selectNbiLocations);
                } else if ($format=='NBIOT') {
                    $selectEutranCellNB = $nbiQuery->getSelectEutranCellNBNbm($locationDim, $timeDim, $nbiFormulaFilter, $hour, $minute, $cell);
                    $ThisDate            = '';
                    for ($i = strtotime($startTime); $i <= strtotime($endTime); $i += 86400) {
                        $ThisDate = $ThisDate.'\''.date("Y-m-d", $i).'\',';
                    }
                    ...
                    $ThisDate = substr($ThisDate, 0, (strlen($ThisDate) - 1));
                    // 筛选项：日期
                    $selectDateId = $selectEutranCellNB." where date_id in ($ThisDate)";
                    // 筛选项：时间维度
                    $selectTimeDim = $nbiQuery->getSelectTimeDimNbm($timeDim, $hour, $minute, $selectDateId);
                    // 筛选项：区域维度
                    $nbiLocation       = $nbiQuery->getLocationNbm($city);
                    ...
                    // 筛选项：基站or小区
                    $selectNbiLocations = $nbiQuery->getSelectNbiLocationNbm($selectNbiLocation, $locationDim, $erbs, $cell);
                    $select = $nbiQuery->getSelectNBNbm($locationDim, $timeDim, $selectNbiLocations);
                }
            } else {
    ...
    public function NBITime()
    {
        ...
        if ($format=="TDD") {
            ...
            $table = 'EutranCellFdd_cell_day';
        } else if ($format=='NBIOT') {
            $table = 'EutranCellNB_cell_day';
        }
        $result = array();
...
class GetItems
{
     ...
     */
    public function getResultText($locationDim, $timeDim, $format)
    {
        ...
        if ($locationDim != "city" && $locationDim != "cellGroup") {
            if ($timeDim == 'day') {
            	if($format == 'FDD') {
 					return "day,city,location,$types";
 				}
            ...
            if ($timeDim == 'day') {
 				if($format == 'FDD') {
 					return "day,city,cellgroup,cellTotal";
 				}
...
class NbiQuery
{
     ...
     */
    public function getSelectTDDNbm($locationDim, $timeDim, $selectNbiLocations)
    {
        if ($locationDim == "erbs") {
            if ($timeDim == "day") {
     ...
     * @return string SELECT字串(NBM)
     */
    public function getSelectFDDNbm($locationDim, $timeDim, $selectNbiLocations)
    {
        if ($locationDim == "erbs") {
            if ($timeDim == "day") {
                $select = $selectNbiLocations."group by date_id,City,ManagedElement";
            ...
            if ($timeDim == "day") {
                $select = $selectNbiLocations."group by date_id,City,EutranCellFdd";
                return $select;
            } else if ($timeDim == "hour") {
                $select = $selectNbiLocations."group by date_id,hour_id,City,EutranCellFdd";
                return $select;
            } else if ($timeDim == "quarter") {
                $select = $selectNbiLocations."group by date_id,hour_id,min_id,City,EutranCellFdd";
                return $select;
            } else if ($timeDim == "hourgroup") {
                $select = $selectNbiLocations."group by date_id,City,EutranCellFdd";
                return $select;
     ...
     * @return string SELECT字串(NBM)
     */
    public function getSelectNBNbm($locationDim, $timeDim, $selectNbiLocations)
    {
        if ($locationDim == "erbs") {
            if ($timeDim == "day") {
                $select = $selectNbiLocations."group by date_id,City,ManagedElement";
            ...
            if ($timeDim == "day") {
                $select = $selectNbiLocations."group by date_id,City,EutranCellNB";
                return $select;
            } else if ($timeDim == "hour") {
                $select = $selectNbiLocations."group by date_id,hour_id,City,EutranCellNB";
                return $select;
            } else if ($timeDim == "quarter") {
                $select = $selectNbiLocations."group by date_id,hour_id,min_id,City,EutranCellNB";
                return $select;
            } else if ($timeDim == "hourgroup") {
                $select = $selectNbiLocations."group by date_id,City,EutranCellNB";
                return $select;
     ...
     * @return string
     */
    public function getSelectEutranCellFDDNbm($locationDim, $timeDim, $nbiFormulaFilter, $hour, $minute, $cell)
    {
        if ($locationDim == "erbs") {
            if ($timeDim == "day") {
                return "select date_id,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellFdd_station_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellFdd_station_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellFdd";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    return "select date_id,'24h' as hourgroup,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellFdd_station_hour";
                } else {
                    return "select date_id,('".$hour."') as hourgroup,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellFdd_station_hour";
                }
            }
        } else if ($locationDim == "cell") {
            if ($timeDim == "day") {
                return "select date_id,city AS City,UserLabel AS EutranCellFdd,count(distinct UserLabel) as ecgiTotal,$nbiFormulaFilter from EutranCellFdd_cell_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,UserLabel AS EutranCellFdd,count(distinct UserLabel) as ecgiTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,UserLabel AS EutranCellFdd,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    // return "select date_id,'24h' as hourgroup,UserLabel AS EutranCellTdd,SUBSTRING_INDEX(Dn,',',1) AS City,$nbiFormulaFilter from EutranCellTdd_hour";
                    return "select date_id,'24h' as hourgroup,UserLabel AS EutranCellFdd,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
                } else {
                    // return "select date_id,('".$hour."') as hourgroup,UserLabel AS EutranCellTdd,SUBSTRING_INDEX(Dn,',',1) AS City,$nbiFormulaFilter from EutranCellTdd_hour";
                    return "select date_id,('".$hour."') as hourgroup,UserLabel AS EutranCellFdd,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
                }
            }
        } else if ($locationDim == "cellGroup") {
            if ($timeDim == "day") {
                return "select date_id,city AS City,('cellgroup') as cellgroup,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,('cellgroup') as cellgroup,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,('cellgroup') as cellgroup,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    return "select date_id,'24h' as hourgroup,('cellgroup') as cellgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
                } else {
                    return "select date_id,('".$hour."') as hourgroup,('cellgroup') as cellgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
                }
            }
        } else {
            if ($timeDim == "day") {
                return "select date_id,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    return "select date_id,'24h' as hourgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
                } else {
                    return "select date_id,('".$hour."') as hourgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellFdd_cell_hour";
                }
            }
        }//end if

    }//end getSelectEutranCellFdd_nbm()
     ...
     * @return string
     */
    public function getSelectEutranCellNBNbm($locationDim, $timeDim, $nbiFormulaFilter, $hour, $minute, $cell)
    {
        if ($locationDim == "erbs") {
            if ($timeDim == "day") {
                return "select date_id,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,'' as hourTotal,$nbiFormulaFilter from EutranCellNB_station_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellNB_station_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellNB";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    return "select date_id,'24h' as hourgroup,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellNB_station_hour";
                } else {
                    return "select date_id,('".$hour."') as hourgroup,city AS City,erbs AS ManagedElement,count(distinct erbs) as erbsTotal,$nbiFormulaFilter from EutranCellNB_station_hour";
                }
            }
        } else if ($locationDim == "cell") {
            if ($timeDim == "day") {
                return "select date_id,city AS City,UserLabel AS EutranCellNB,count(distinct UserLabel) as ecgiTotal,'' as hourTotal,$nbiFormulaFilter from EutranCellNB_cell_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,UserLabel AS EutranCellNB,count(distinct UserLabel) as ecgiTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,UserLabel AS EutranCellNB,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    // return "select date_id,'24h' as hourgroup,UserLabel AS EutranCellTdd,SUBSTRING_INDEX(Dn,',',1) AS City,$nbiFormulaFilter from EutranCellTdd_hour";
                    return "select date_id,'24h' as hourgroup,UserLabel AS EutranCellNB,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
                } else {
                    // return "select date_id,('".$hour."') as hourgroup,UserLabel AS EutranCellTdd,SUBSTRING_INDEX(Dn,',',1) AS City,$nbiFormulaFilter from EutranCellTdd_hour";
                    return "select date_id,('".$hour."') as hourgroup,UserLabel AS EutranCellNB,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
                }
            }
        } else if ($locationDim == "cellGroup") {
            if ($timeDim == "day") {
                return "select date_id,city AS City,('cellgroup') as cellgroup,count(distinct UserLabel) as cellTotal,'' as hourTotal,$nbiFormulaFilter from EutranCellNB_cell_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,('cellgroup') as cellgroup,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,('cellgroup') as cellgroup,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    return "select date_id,'24h' as hourgroup,('cellgroup') as cellgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
                } else {
                    return "select date_id,('".$hour."') as hourgroup,('cellgroup') as cellgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
                }
            }
        } else {
            if ($timeDim == "day") {
                return "select date_id,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_day";
            } else if ($timeDim == "hour") {
                return "select date_id,hour_id,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
            } else if ($timeDim == "quarter") {
                return "select date_id,hour_id,min_id,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB";
            } else if ($timeDim == "hourgroup") {
                if ($hour == '') {
                    return "select date_id,'24h' as hourgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
                } else {
                    return "select date_id,('".$hour."') as hourgroup,city AS City,count(distinct UserLabel) as cellTotal,$nbiFormulaFilter from EutranCellNB_cell_hour";
                }
            }
        }//end if

    }//end getSelectEutranCellNB_nbm()
    ...
    public function parseNbiKpiPre($nbiKpis)
    {
        ...
        // print_r($nbiKpiFormulaData);return;
        $dbc = new DataBaseConnection();
        $db = $dbc->getDB('mongs', 'mongs');
        $res = $db->query($nbiKpiFormulaData);
    ...
    public function parseNbiKpiTest($nbiKpis)
    {
        ...
        $nbiKpiFormulaData = "select kpiFormula,instr('$nbiKpisIds',CONCAT(id,',')) as sort  from kpiFormulaNbi where id in($nbiKpisIds) ORDER BY field(id,$nbiKpisIds)";
        ...
        $i   = 0;
        $row = [];
        while ($rows = $res->fetch(PDO::FETCH_NUM)) {
        // while ($rows = mysql_fetch_row($res)) {
            $row[$i++] = $rows[0];
        }

        //判断counter是否存在
        $judge = $this->getCheckCounters($row);
        ...
        $newArr['nbiKpiFormula'] = $nbiKpiFormula;
        $newArr['judge'] = $judge;
        return $newArr;
    }//end parseNbiKpiTest()
    ...
    /**
    */
    function getCheckCounters($rows) 
    {
        $noCounters = [];
        $file = fopen('common/txt/Counters_nbm.txt', 'r');
        $counters = [];
        while (!feof($file)) { 
            array_push($counters, explode('=', fgets($file))[0]);
        }
        array_unique($counters);
        foreach ($rows as $value) { 
            $pattern       = "/[\(\)\+\*-\/]/";
            $columns       = preg_split($pattern, $value);
            foreach ($columns as $column) {
                $pattern = '/[^max][^log10][^power][a-zA-Z_0-9]+/';
                ...
                foreach ($matches as $match) {
                    ...
                    $flag = 0;
                    $data = array_search($match, $counters);
                    if ( $data ) {
                        $flag = 1;
                        continue;
                    }
    ...
    public function getNbiKpis()
    {
        ...
        $queryKpiset  = "select elementId from templateNbi where id='$templateId'";
        $nbiKpis = $db->query($queryKpiset)->fetchAll(PDO::FETCH_ASSOC);
        // print_r($nbiKpis[0]['elementId']);
        // $res          = mysql_query($queryKpiset);
        // $nbiKpis      = mysql_fetch_array($res);
        $nbiKpisNum   = $nbiKpis[0]['elementId'];
        $nbiKpisNumStr = $nbiKpisNum . ',';
        $nbiKpisNumStr = rtrim($nbiKpisNumStr, ",");
        // $queryNbiKpiName = "select kpiName,instr('$nbiKpisNumStr',CONCAT(id,','))  as sort from kpiFormulaNbi where id in ($nbiKpisNum) ORDER BY sort";
        $queryNbiKpiName = "select kpiName,instr('$nbiKpisNumStr',CONCAT(id,','))  as sort from kpiFormulaNbi where id in ($nbiKpisNum) ORDER BY field(id,$nbiKpisNum)";
        // $queryNbiKpiName = "select kpiName,instr('$nbiKpisNum',id) as sort from kpiFormulaNbi where id in ($nbiKpisNum) ORDER BY sort";
        $query = $db->query($queryNbiKpiName);
