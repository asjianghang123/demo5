...
// use App\Http\Controllers\QueryAnalysis\Page;
use SubnetWork;
 ...
 * @link     https://laravel.com/docs/5.4/controllers
 */
class LTEQueryController extends GetTreeData
{
    ...
    public function templateQuery()
    {
        ...
        if ($is_neigh[0]['description'] != 'neighborCell'||count($citys)>1) {
            ...
            if(!is_array($is_2oss) && substr($is_2oss, 0, '6') == 'check:') {
                return json_encode(array('error' => $is_2oss));
            }
...
class OssGetTogether
{
        ...
        if ($locationDim == 'city' || $locationDim == 'subNetworkGroup' || $locationDim == 'erbsGroup' || $locationDim == 'cellGroup') {
            ...
            foreach ($citysChinese as $numCity) {
                ...
                foreach (Databaseconns::where("cityChinese", $numCity)->cursor() as $flight) {
                    ...
                    if(!is_array($ossGetTogether) && substr($ossGetTogether, 0, '6') == 'check:') {
                        return $ossGetTogether;
                    }
                    //合并数据
                ...
                if (count($mergeArr) == 1 && count($mergeArr)!=0) {  
                    ...
                    for ($i=0; $i < count($arr); $i++) { 
                        for ($j=0; $j < count($arr[$i]); $j++) { 
                            $key = $arr[$i][$j]['day'];
                            if (array_key_exists('hour', $arr[$i][$j])) {
                                $key .= $arr[$i][$j]['hour'];
                            }
                            if (array_key_exists('minute', $arr[$i][$j])) {
                                $key .= $arr[$i][$j]['minute'];
                            }
                            /*if (array_key_exists('location', $arr[$i][$j])) {
                                $key .= $arr[$i][$j]['location'];
                            }*/
                            if (count($arrkey)>=1) {
                                if(array_key_exists($key, $arrkey)){
                                    foreach ($newArrs[$key] as $k => $value) {
                                        $flag = 0;
                                        if ($k == 'day' || $k == 'hour' || $k == 'minute' || $k == 'location') {
                                            ...
                                            if ($flag == 1) {
                                                $newArrs[$key][$k] = max($value, $arr[$i][$j][$k]);
                                            } else {
                                                $newArrs[$key][$k] = $value + $arr[$i][$j][$k];
                                            }  
                                        }
                                    }
                                }else {
                ...
                if(Input::get("format") == "FDD") {
                    $return['text'] = str_replace("cellNum", "siteNum", $return['text']);
                }
        ...
        $endTime, $city, $subNets, &$resultText, $aggTypes, $format, $filename, $numOf_1000000, $parent)
    {
        ...
        // print_r($sql);exit;
        // print_r($sql);return;
        if (!is_array($sqlArr) && substr($sqlArr, 0, '6') == 'check:') {
            $checkRows = Kpiformula::select('kpiName','kpiFormula')
            ->whereIn('id', explode(',', $kpis['ids']))
            ->WHERE('user', $parent)
            ->get()
            ->toArray();
            $wrongRows = explode(',', explode(':', $sqlArr)[1]);
            $wrongarr = [];
            foreach ($wrongRows as $key => $value) {
                foreach ($checkRows as $k => $v) {
                    $pos = strpos($v['kpiFormula'], $value);
                    if($pos!==false) {
                        $wrongarr[$v['kpiName']][] = $value;
                    }
                }
             } 
             $sql = "check:error<br/>公式=>指标<br/>";
             foreach ($wrongarr as $key => $value) {
                 $sql .= '['.$key.']=>';
                 foreach ($value as $v) {
                     $sql .= $v . '/';
                 }
                 $sql = substr($sql,0,strlen($sql)-1)."<br/>"; 
             }
            return $sql;
        }
        $sql = $sqlArr['sql'];
    ...
    protected function createSQL($localDB, $pmDB, $kpiName, $counters, $timeDim, $locationDim,
        ...
        $isAllSubnets    = str_replace("]","", $isAllSubnets);
        $rows = [];
        $subNetworkarray = explode(",", str_replace('"', "", $isAllSubnets));
        $subNetworkstring = "";
        if($format == "TDD") {
            $rows   = $localDB->query("SELECT subNetwork FROM databaseconn WHERE connName='$city'")->fetchAll(PDO::FETCH_ASSOC);
            foreach ($subNetworkarray as $value) {
            	if (strpos($rows[0]["subNetwork"], $value) !== FALSE) {
            		$subNetworkstring = $subNetworkstring . '"'.$value . '",';
            	} 
            }
            $subNetworkstring = rtrim($subNetworkstring, ",");
            ...
            if($isAllSubnets != $chooseSub) {
                $rows[0]['subNetwork'] = $subNetworkstring; 
            }
        }elseif($format == "FDD") {
        ...
        if ($hour != 'null' && ($timeDim == "hour" || $timeDim == "quarter" || $timeDim == "hourgroup"|| $timeDim == "day" ||$timeDim=='daygroup')) {
            $hour     = "(".$hour.")";
        ...
        $sql = str_replace("\n", "", $sql);
        $sql = SubnetWork::is_ReplaceDc($sql, $city);
        $return['sql'] = $sql;    
    ...
    function createTempTable($locationDim, $selectSQL, $whereSQL, $tableName, $counters, $groupSQL, $aggTypes, $nosum_map, $counterMap, $format, &$resultText
        ...
        if (strpos($tableName, "TDD") !== false) {
            $whereSQL = str_replace("FDD", "TDD", $whereSQL);
        }
    ...
    protected function getCitySQL($city)
    {
        ...
        // return Page::provinceMatch(getenv('CITY'))['SN'];
        $SN = SubnetWork::getSubnetWork($city);
        return $SN;
...
class LoadCounters
{
    ...
    protected function loadCountersFromFile()
    {
        ...
        foreach ($lines as $line) {
            $pair = explode("=", $line);
            if (input::get('format')=="NBIOT") {
                if (trim($pair[1])=='DC_E_ERBS_NBIOTCELL_V'||trim($pair[1])=='DC_E_ERBS_NBIOTCELL'||trim($pair[1])=='DC_E_ERBS_ENODEBFUNCTION') {
...
class LTEQuery
{
    ...
    public function queryTemplate($localDB, $pmDB, $counters, $timeDim, $locationDim, $startTime,
        ...
        if (substr($sql, 0, '6') == 'check:') {
            ...
            ->get()
            ->toArray();
            $wrongRows = explode(',', explode(':', $sql)[1]);
            $wrongarr = [];
            foreach ($wrongRows as $key => $value) {
                foreach ($checkRows as $k => $v) {
                    $pos = strpos($v['kpiFormula'], $value);
                ...
                while ($rows = $res->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ($locationDim == 'cell') {
                        ...
                        if (Cache::store('file')->has($cell)&&input::get('format')!='NBIOT') {
                            ...
                            $sql = "SELECT cellNameChinese,cluster,ecgi,siteType,siteNameChinese,覆盖属性 FROM siteLte WHERE cellName='$cell'";
                            ...
                            $cacheValue = $cacheCn . ',' . $cacheCluster . ',' . $cacheSiteType . ',' . $cacheSiteNameChinese . ',' . $cacheCoverage . ',' . $cacheEcgi;
                ...
                while ($rows = $res->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ($locationDim == 'cell') {
                        ...
                        if (Cache::store('file')->has($cell)) {
                            ...
                            $cacheSiteName = '';
                            $cacheEcgi = '';
                            $sql = "SELECT cellNameChinese,cluster,siteNameChinese,ecgi FROM siteLte WHERE cellName='$cell'";
                            $rs = $dbc->query($sql)->fetchAll(PDO::FETCH_ASSOC);
                            if (count($rs)==0) {
                                $cacheCn = '无数据';
                                $cacheCluster = '无数据';
                            ...
                            $cacheValue = $cacheCn . ',' . $cacheCluster . ',' . $cacheSiteName . ',' . $cacheEcgi;
                            Cache::store('file')->forever($cell, $cacheValue);
                        }
                        $cn = explode(',', $cacheValue)[0];
                        $cluster = explode(',', $cacheValue)[1];
                        $siteNameChinese = explode(',', $cacheValue)[2];
                        $ecgi = explode(',', $cacheValue)[3];
                    } elseif ($locationDim == 'erbs') {
                        $erbs = $rows['location'];
                        if (Cache::store('file')->has($erbs)) {
                            $cacheValueErbs = Cache::store('file')->get($erbs);
    ...
    protected function createSQL($localDB, $pmDB, $kpiName, $counters, $timeDim, $locationDim,
        ...
        if ($timeDim == "daygroup") {
            ...
            if ($local_flag) {
                ...
                // $selectSQL    = $selectSQL." convert(char(10),date_id) as day,";
                // if($inputHour != 'null') {
                //     $selectSQL = $selectSQL." '".$hour . "' as hour,";
                //     $aggSelectSQL = "select AGG_TABLE0.day,AGG_TABLE0.hour,AGG_TABLE0.cellNum";
                //     $resultText   = $resultText."day,hour,cellNum,";
                // }else {
                //     $aggSelectSQL = "select AGG_TABLE0.day,AGG_TABLE0.cellNum";
        ...
        if ($locationDim == "city") {
            ...
            if ($local_flag) {
                ...
                if(input::get('format')=='NBIOT'){
                    // $selectSQL = $selectSQL."$citySQL as subNet,substring(substring(SN,charindex (',', substring(SN, 32, 25)) + 32),11,25) as site, NbIotCell as location,";
                    // $selectSQL = $selectSQL."$citySQL as subNet,".Page::provinceMatch(getenv('CITY'))['site']." as site, NbIotCell as location,";
                    $selectSQL = $selectSQL."$citySQL as subNet,".SubnetWork::getSN(input::get('format'), $city).", NbIotCell as location,";
                }
                else {
                    // $selectSQL = $selectSQL."$citySQL as subNet,substring(substring(SN,charindex (',', substring(SN, 32, 25)) + 32),11,25) as site,EutranCellTDD as location,";
                    // $selectSQL = $selectSQL."$citySQL as subNet,".Page::provinceMatch(getenv('CITY'))['site']." as site,EutranCellTDD as location,";
                    $selectSQL = $selectSQL."$citySQL as subNet,".SubnetWork::getSN(input::get('format'), $city).",EutranCellTDD as location,"; 
                }
            }
            ...
            if ($timeDim == 'daygroup') {
                ...
                $resultText = $resultText."subNet,cluster,site,siteType,sitenamechinese,location,cellNameChinese,ecgi,";
            } else {
                $resultText = $resultText."subNet,cluster,site,siteType,sitenamechinese,location,cellNameChinese,覆盖属性,ecgi,";
            }
            // $resultText = $resultText."subNet,site,location,";
        } else if ($locationDim == "cellGroup") {
            $aggSelectSQL = $aggSelectSQL.",";
        ...
        $inputErbs = Input::get('erbs');
        $erbs      = isset($inputErbs) ? $inputErbs : "";
        if ($erbs != "" && ($locationDim == "erbs" || $locationDim == "erbsGroup" || $locationDim == "cell")) {
            $erbs     = "('".str_replace(",", "','", $erbs)."')";
            $whereSQL = $whereSQL." and erbs in ".$erbs;
        }
        ...
        foreach ($tables as $table) {
            ...
            if ($index == 0) {
                ...
                if ( $timeDim == "daygroup" && $locationDim == 'cell' ) {
                    $tableSQL = $tableSQL." and AGG_TABLE0.location = AGG_TABLE$index.location";
                }
                // echo $locationDim;
                if ($locationDim == "cellGroup" || $timeDim == "daygroup"  || $locationDim == "erbsGroup") {
    ...
    public function parseLocation($city, $subNetwork)
    {
        ...
        foreach ($citys as $city) {
            ...
            $connCityChineses  = $databaseConns[0]->cityChinese;
           
            // var_dump($city);
            if ($format == 'TDD') {
                    $subStr = $databaseConns[0]->subNetwork;
                } else if ($format == 'FDD') {
                    $subStr = $databaseConns[0]->subNetworkFdd;
                }else if($format=='NBIOT'){
                    $subStr = $databaseConns[0]->subNetworkNbiot;
                }
            $connCityChineses = explode(',', $connCityChineses);
            $subArr           = explode(',', $subStr);
            $newSubNetworks   = '';
            ...
            // foreach($connCityChineses as $connCityChinese){
            //     if ($item['connName'] == ) {
