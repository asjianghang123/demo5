...
use App\Models\Mongs\Databaseconns_HW;
 ...
 * @link     https://laravel.com/docs/5.4/controllers
 */
class LTEQueryHWController extends GetTreeData
{
    ...
    public function getTreeData()
    {
        ...
        if ($login_user == "admin") {
            ...
            $users      = Template_HW::whereIn("user", [$login_user,"admin"])->distinct('user')->get(['user']);
        }
        $arrUser    = array();
        $items      = array();
        $itArr      = array();
    ...
    public function getAllCity()
    {
        ...
        return $cityClass->getCityOptions_HW();
    ...
    public function getFormatAllSubNetwork()
    {
        ...
        foreach ($citys as $city) {
            $databaseConns = Databaseconns_HW::where('cityChinese', $city)->get();
    ...
    public function templateQuery()
    {
        ...
        $citys  = $cityPY->getConnCityHW($citysChinese);
        ...
        //目前不支持NBIOT/包含邻区对
        /*$is_neigh = Template_HW::select('description')
        ...
        if($is_neigh[0]['description'] != 'neighborCell' && $format != 'NBIOT') {
            ...
            $is_2oss = $ossGetTogether->is_2oss($db, $counters, $timeDim, $locationDim, $startTime, $endTime, $citysChinese, $aggTypes, $format, $filename, $numOf_1000000, $parent, $template, $timeSearchStart, $time_monitor);
    ...
    public function getElementTree()
    {
        ...
        $elementId = Template_HW::where('id', $templateId)->first()->toArray();
...
{
    public function is_2oss($db, $counters, $timeDim, $locationDim, $startTime, $endTime, $citysChinese, $aggTypes, $format, $filename, $numOf_1000000, $parent, $template, $timeSearchStart, $time_monitor) {
        $return = [];
        $items = [];
        $columnMax = [];
        if ($locationDim == 'city' || $locationDim == 'subNetworkGroup' || $locationDim == 'erbsGroup' || $locationDim == 'cellGroup') {
            ...
            foreach ($citysChinese as $numCity) {
                ...
                // if($count > 1) {
                $mergeArr = [];
                foreach (Databaseconns_HW::where("cityChinese", $numCity)->cursor() as $flight) {
                    $host = $flight->host;
                    $port = $flight->port;
                ...
                //获取合并计算后的数据
                $getTextAndRows = $this->getTextAndRows($db, $template, $parent, $testArr, $locationDim, $timeDim);
    ...
    protected function getTextAndRows($db, $template, $parent, $testArr, $locationDim, $timeDim) 
    {
    ...
    protected function getKpis_1($db, $template, $parent) 
    {
        ...
        $kpisStr      = $kpis.',';
        $queryKpiName = "select kpiName,instr('$kpisStr',CONCAT(id,',')) as sort from kpiformula_HW where id in ($kpis) order by sort";
        ...
        $kpisName = $kpis.',';
        $queryFormula  = "select kpiName,kpiFormula,kpiPrecision,instr('$kpisName',CONCAT(id,',')) as sort from kpiformula_HW where id in (".$kpis.") order by sort";
    ...
    ) 
    {
        ...
        $kpis          = "";
        $kpiNameStr    = $kpiName.',';
        $queryFormula  = "select kpiName,kpiFormula,kpiPrecision,instr('$kpiNameStr',CONCAT(id,',')) as sort from kpiformula_HW where id in ".$kpiset." order by sort";
        $index         = 0;
        $selectSQL     = "select";
        $counterMap    = array();
        $nosum_map     = array();
        ...
        foreach ($tables as $table) {
            ...
            $tableSQL         = $this->createTempTable($locationDim, $selectSQL, $whereSQL, $table, $countersForQuery, $aggGroupSQL, $aggTypes, $nosum_map, $counterMap, $format);
    ...
    function createTempTable($locationDim, $selectSQL, $whereSQL, $tableName, $counters, $groupSQL, $aggTypes, $nosum_map, $counterMap, $format
    ) {
        $tables = array_keys(array_count_values($counterMap));
        $flag   = 'true';
        $flag1  = 'true';
...
class LoadCounters
{
    ...
    {
        if (file_exists("common/txt/HWCounters.txt")) {
    ...
    {
        $result = array();
        $lines  = file("common/txt/HWCounters.txt");
...
class LTEQuery
{
    ...
    public function getKpis($localDB, $templateName, $parent)
    {
        $queryKpiset  = "select elementId from template_HW where templateName='$templateName' and user = '$parent'";
        $res          = $localDB->query($queryKpiset);
        $kpis         = $res->fetchColumn();
        $kpisStr      = $kpis.',';
        $queryKpiName = "select kpiName,instr('$kpisStr',CONCAT(id,',')) as sort from kpiformula_HW where id in ($kpis) order by sort";
    ...
    protected function getCheckCounters($queryFormula, $counters, $localDB) 
    {
        ...
        foreach ($localDB->query($queryFormula) as $row) {
            ...
            foreach ($columns as $column) {
                $column      = trim($column);
                $counterName = $column;
                if (stripos($counterName, "L_") === false) {
                    // array_push($check, $counterName);
                    $pattern = '/[^max][^log10][^power][a-zA-Z]+/';
                    preg_match($pattern, $counterName, $matches);
                    if(count($matches) != 0)
    ...
    ) 
    {
        ...
        foreach ($localDB->query($queryFormula) as $row) {
            ...
            // $formula = "cast(".$kpi." as decimal(18,".$row['kpiPrecision']."))";
            $formula = "round(".$kpi." ,".$row['kpiPrecision'].")";
            $kpis    = $kpis.$formula." as kpi".$index.",";
            $index++;
        }
        ...
        if ($timeDim == "daygroup") {
            ...
            if ($local_flag) {
                ...
                // $selectSQL    = $selectSQL." convert(char(10),date_id) as day,";
                $selectSQL    = $selectSQL." $time_id as day,";
                // $aggSelectSQL = "select AGG_TABLE0.day,AGG_TABLE0.cellNum";
                $aggSelectSQL = "select AGG_TABLE0.day";
                // $resultText   = $resultText."day,cellNum,";
                $resultText   = $resultText."day,";
            ...
            if ($local_flag) {
                ...
                $selectSQL    = $selectSQL." $time_id as day,$hour_id as hour,";
                // $aggSelectSQL = "select AGG_TABLE0.day,AGG_TABLE0.hour,AGG_TABLE0.cellNum";
                // $resultText   = $resultText."day,hour,cellNum,";
                $aggSelectSQL = "select AGG_TABLE0.day,AGG_TABLE0.hour";
                $resultText   = $resultText."day,hour,";
            ...
            $aggGroupSQL = "group by $time_id,$hour_id,";
            $aggOrderSQL = "order by AGG_TABLE0.day,AGG_TABLE0.hour";
        } else if ($timeDim == "quarter") {
            $selectSQL    = $selectSQL." convert(char,date_id) as day,hour_id as hour,min_id as minute,";
            $aggGroupSQL  = "group by date_id,hour_id,min_id,";
            ...
            if ($local_flag) {
                ...
                // $selectSQL    = $selectSQL." convert(char(10),date_id) as day,\"$hourcollection\" as hour,";
                $selectSQL    = $selectSQL." $time_id as day,\"$hourcollection\" as hour,";
                // $aggSelectSQL = "select AGG_TABLE0.day,AGG_TABLE0.hour,AGG_TABLE0.cellNum";
                // $resultText   = $resultText."day,hour,cellNum,";
                $aggSelectSQL = "select AGG_TABLE0.day,AGG_TABLE0.hour";
                $resultText   = $resultText."day,hour,";
            ...
            $aggGroupSQL = "group by $time_id,hour,";
            $aggOrderSQL = "order by AGG_TABLE0.day,AGG_TABLE0.hour";
        }//end if
        // print_r($selectSQL);return;
        if ($locationDim == "city") {
            ...
            $selectSQL    = $selectSQL."$citySQL as subNet,FunctionName as location,";
            $aggSelectSQL = $aggSelectSQL.",AGG_TABLE0.subNet,AGG_TABLE0.location,";
            if ($local_flag) {
            ...
            if ($local_flag) {
                ...
                if (input::get('format')=='NBIOT') {
                    ...
                    // $selectSQL = $selectSQL."$citySQL as subNet,substring(substring(SN,charindex (',', substring(SN, 32, 25)) + 32),11,25) as site,EutranCellTDD as location,";
                    $selectSQL = $selectSQL."$citySQL as subNet,FunctionName as site,CellName as location,";
                }
            }
            ...
            if ($timeDim == 'daygroup') {
                $resultText = $resultText."userLabel,FunctionName,CellName,";
            } elseif (input::get('format') == 'FDD') {
                $resultText = $resultText."subNet,cluster,site,siteType,sitenamechinese,location,cellNameChinese,";
            } else {
                // $resultText = $resultText."subNet,cluster,site,siteType,sitenamechinese,location,cellNameChinese,覆盖属性,";
        ...
        if ($hour != 'null' && ($timeDim == "hour" || $timeDim == "quarter" || $timeDim == "hourgroup")) {
            $hour     = "(".$hour.")";
            // $whereSQL = $whereSQL." and hour_id in ".$hour;
            $whereSQL = $whereSQL." and hourid in ".$hour;
        }
        ...
        $dbc      = new DataBaseConnection();
        $db       = $dbc->getDB('mongs', 'mongs');
        $sql      = "select description from template_HW where templateName='$templateNameCheck'";
        $res      = $db->query($sql);
        $rows     = $res->fetchAll(PDO::FETCH_ASSOC);
